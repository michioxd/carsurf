...
ssh_cmd=("${auth[@]}" ssh "${ssh_options[@]}" "$remote")
scp_cmd=("${auth[@]}" scp "${ssh_options[@]}")
...
main_identifier=$(remote_exec "/var/jb/usr/bin/ldid -h $(shell_quote "$main_binary") 2>/dev/null | grep '^Identifier=' | cut -d= -f2- | head -n 1")
[[ -n $main_identifier ]] || {
...
remote_exec "/var/jb/usr/bin/ldid -S$remote_tmp/main-entitlements.plist -I$(shell_quote "$main_identifier") $(shell_quote "$main_binary")"
...
remote_exec "jbctl trustcache add $main_hash"
...
for ((index=0; index<${#framework_paths[@]}; index++)); do
    ...
    framework_actions+=$'\n'
    framework_actions+="jbctl trustcache add ${framework_hashes[$index]}"

    # If dyld mapped this image before it was trusted, the kernel can retain a
    # non-platform csblob for its vnode. Replace only the inode with an archival
    # copy after trusting the unchanged CDHash, forcing a fresh classification.
    # The embedded bytes, code signature, owner, mode, timestamps and xattrs stay
    # intact. Staging beside the original also keeps the replacement on one APFS
    # volume so mv is atomic.
    framework_path=${framework_paths[$index]}
    staged_framework="$framework_path.carsurf-trusted-$timestamp"
    framework_actions+=$'\n'
    framework_actions+="cp -a $(shell_quote "$framework_path") $(shell_quote "$staged_framework")"
    framework_actions+=$'\n'
    framework_actions+="mv -f $(shell_quote "$staged_framework") $(shell_quote "$framework_path")"
done
